...
export default function Experience() {
  function Rig() {
    return useFrame((state, delta) => {
      easing.damp3(state.camera.position, [0 + state.mouse.x / 1, 1.5 + state.mouse.y / 1, 30], 0.2, delta)
      easing.damp3(state.camera.rotation, [0 , -Math.PI/180 * 0 + 2 * Math.PI/180 * 4 *state.mouse.x, 0], 0.2, delta)
